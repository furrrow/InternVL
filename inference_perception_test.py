...
def collate_fn(batch):
    ...
    n_patches = [x['num_patches_list'] for x in batch]
    ...
    return vid, n_patches, v_id, ins, q_ids, ops, ans
...
def run_inference(args):
    ...
    for i, (video_tensor, num_patches_list, video_id, instructs, question_ids, options, true_ans) in enumerate(tqdm(dataloader)):

        # reduce batch dimension
        pixel_values = video_tensor[0]
        pixel_values = pixel_values.to(torch.bfloat16).cuda()
        ...
        for idx, question in enumerate(instructs):
            ...
            warmup_question = video_prefix + 'what is happening?'
            # "Frame1: <image>\nFrame2: <image>\n...\nFrame8: <image>\n{question}"
            response, history = model.chat(tokenizer, pixel_values, warmup_question, generation_config,
                                           num_patches_list=num_patches_list, history=None, return_history=True)
            # print(f'User: {warmup_question}\nAssistant: {response}')
            # print("\n ---- \n")
            ...
            output = response.replace('answer', '')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--answer_file", type=str, required=False, default="./answer_file.json")
